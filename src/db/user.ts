...
export interface User {
  ...
  role: UserRole;
  peopleUserId?: number | null; // Legacy field for People API integration
...
export interface CreateUserData {
  username: string;
  displayName: string;
  email: string; // Required - primary login method
  phone?: string; // Optional
  pin?: string; // Optional 4-digit PIN (will be hashed before storage)
...
export type CreateUserWithPhoneData = CreateUserData;

export async function createUserWithEmail(data: CreateUserData): Promise<User> {
  const id = uuidv4();
  const now = new Date();
  
  // Hash the PIN before storing (only if provided)
  ...
  console.log('🆕 [CREATE USER] Created user:', {
    userId: id,
    username: data.username,
    email: data.email,
    phone: data.phone || null,
...
export async function createUserWithPhone(data: CreateUserData): Promise<User> {
...
export async function updateUserPublicAddress(userId: string, publicAddress: string): Promise<void> {
  await db
    .update(users)
    .set({ accountAddress: publicAddress, updatedAt: new Date() })
    .where(eq(users.id, userId));
}
