 ...
 * 
 * Step 1: POST /api/auth/email-login with { email } only
 ...
 * 
 * Step 2: POST /api/auth/email-login with { email, pin }
...
export default async function handler(
  ...
  try {
    const { email, pin, pendingUserData } = req.body as { 
      email?: string; 
      pin?: string;
    ...
    if (!pinValid) {
      ...
      if (wasLocked) {
        console.log('🔒 [EMAIL LOGIN] Account locked due to failed PIN attempts:', {
          userId: user.id,
      ...
      console.log('❌ [EMAIL LOGIN] Invalid PIN:', {
        userId: user.id,
        username: user.username,
        attemptsRemaining,
      });
    ...
    if (pendingUserData?.accountAddress) {
      console.log('🔗 [EMAIL LOGIN] Linking accountAddress to user:', {
        userId: user.id,
    ...
    console.log('✅ [EMAIL LOGIN] User logged in successfully:', {
